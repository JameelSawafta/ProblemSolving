 ...
 * Date: 4/4/2022
 * Time: 9:20 AM
 * ProblemLink: https://codeforces.com/problemset/problem/765/A
 */
...
public class NeverendingCompetitions {
    ...
    static void solve() throws Exception {
        int t=scanInt();
        char[] ans=scanString().toCharArray();
        int s = 0;
        for (int i = 0; i < t; i++) {
            char[] en=scanString().toCharArray();
            if (en[0]==ans[0]&&en[1]==ans[1]&&en[2]==ans[2]) s++;
            else if (en[5]==ans[0]&&en[6]==ans[1]&&en[7]==ans[2]) s--;
        }
        if (s==0) System.out.println("home");
